...
const EntradasNumeroCat = () => {


  const [vendadata, setVendadata] = useState([]);
  const [buscavenda, setBuscaVend] = React.useState("")

  const buscarap = buscavenda;

  var table = vendadata.filter(item => item.vendan.includes(buscarap))
...
function somar() {
      ...
      let somaTotal = 0; 
       ...
       if(buscavenda === ""){
        ...
        toast.warning("Campo Busca por Venda vazio ! ...")
           ...
           for (let i = 0; i < linhas.length; i++) {
                ...
                for (let k = 17; k < celulas.length; k++) {
  
                     const valorCat = celulas[k].innerHTML;
      ...
      <div className="row flex-nowrap">
        ...
        <div className="col p-0 m-0">
          ...
          <div className="px-5 mt-5">
            <div className="mb3">
              <label htmlFor="Nome" className="Nome" style={{ fontFamily: 'arial', fontSize: '22px', fontWeight: 'bold' }}>Busca por Venda nº:</label><br />
              <input type="search" autoFocus='true' className="consultacategoria" value={buscavenda} onChange={(e) => setBuscaVend(e.target.value)} style={{ fontFamily: 'arial', fontSize: '22px', fontWeight: 'bold', color: 'navy', width:'150px' }} />
              <Link to="/entradas" className="btn btn-success" style={{ fontSize: '18px', width: '140px', margin: '0 20px' }}>Voltar:</Link>
              <Link onClick={somar} className="btn" style={{ color: 'white', backgroundColor: 'gray', margin: '0 25px', fontSize: '18px'}}>Total Nota Fiscal:</Link>
              <strong style={{ fontSize: '30px' }}>Total:</strong>
              <strong><span id="total" style={{ color: 'green', fontSize: '32px', margin: '0 10px' }}></span></strong><br />
              <label htmlFor="categoria" className="categoria" style={{ fontFamily: 'arial', fontSize: '22px', fontWeight: 'bold', margin: '0 5px' }}>Categoria:</label>
              <br />
              <input type="text" className="vendan" value={categoria} onChange={(e) => setCategoria(e.target.value)} style={{ fontFamily: 'arial', fontSize: '22px', fontWeight: 'bold', color: 'navy', width: '150px', margin: '0 10px' }} />
            </div><br />
...
export default EntradasNumeroCat